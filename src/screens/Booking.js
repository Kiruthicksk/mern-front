...
function Booking() {
 ...
 const handleAddSchedule = async () => {
    try {
      const { data } = await Axios.post("https://ghi-q4ec.onrender.com/admin/add-schedule", {
        flightId,
  ...
  const fetchSchedules = async () => {
    try {
      const { data } = await Axios.get("https://ghi-q4ec.onrender.com/admin");
      setSchedules(data);
    } catch (err) {
      toast.error(getError(err));
    }
  ...
  const handleBook = async () => {
   ...
   const {data} = await Axios.post(`https://ghi-q4ec.onrender.com/customer/add/${userId}`, {
